...
import { CheckRole, UpdateProfileFormDataType, userDataType } from "../../features/helpers/typeGuard.helper";
...
const EditProfileModal = ({userData, setOpenEditProfileModal, openEditProfileModal}: EditProfileModalProp) => {
    ...
    // Get useSubmit Hook
    const {submitForm, isLoading} = useSubmitForm(formData as UpdateProfileFormDataType)
    ...
    const handleSubmit = useCallback(async () => {
        try {
            ...
            if (err.status !== 500){
                ...
                toast.error('Profile Update Failed')
                navigate('/error', {state: {from: location, errorMsg: ['Error occured while trying to update', 'Go back and try again']}})
            } 
        }
    }, [navigate, isLoading, submitForm, setOpenEditProfileModal])
                ...
                {isLoading ? <Spinner loading={isLoading} /> :
                    <div className={`container relative mx-auto w-[95%] px-2 h-[80%] bg-backgroundColor rounded-md md:px-8 py-10 md:w-[50%] md:h-[90%] overflow-y-auto animate-showModal`} ref={modalRef}>
                        <div>
                            ...
                            <FaX fontSize={'22px'} className="absolute right-[8%] top-[5%] cursor-pointer" onClick={() => setOpenEditProfileModal(false)}/>
                        </div>
                        <div className="w-[85%] mx-auto text-xl text-fontDarkColor mt-10">
                            <form onSubmit={handleSubmit} className="flex flex-col gap-y-4">
                                ...
                                <div className="flex flex-col gap-y-2">
                                    ...
                                    <input type="text" value={formData.user?.email || ''} className="py-2 px-3 border text-xl bg-white text-gray-500 rounded-lg" onChange={(e) => setFormData({...formData, user: {...formData.user, email: e.target.value}})}/>
                                </div>
                                <div className="flex flex-col gap-y-2">
                                    <label htmlFor="phoneNumber">Phone Number:</label>
                                    <input type="text" value={formData.user?.phoneNumber || ''} className="py-2 px-3 border text-xl bg-white text-gray-500 rounded-lg" onChange={(e) => setFormData({...formData, user: {...formData.user, phoneNumber: e.target.value}})}/>
                                </div>
                                ...
                                <div className="mt-8 flex justify-between">
                                    ...
                                    <button className="py-2 px-3 bg-slate-400 text-fontDarkColor rounded-lg transition-all delay-75 ease-in-out duration-300 hover:bg-slate-500" onClick={() => setOpenEditProfileModal(false)}>Cancel</button>
                                </div>
                            </form>
                        </div>
